...
from data.models import Integration, IntegrationServiceName, User, BeelineCall
...
def process_account_id(
    ...
    try:
        ...
        for call in calls:
            beeline_call = BeelineCall.get_or_none(
                call_id=call.id_
            )
            if beeline_call is not None:
                logger.info(f"[-] Beeline: {integration.account_id}. "
                            f"Запись уже обрабатывалась: {call.id_}")
                continue
            try:
                BeelineCall.create(
                    call_id=call.id_,
...
def main():
    account_ids = ['president', 'missier_pirsing', 'bassein-service']
    ...
    while True:
        ...
        for account_id in account_ids:
            new_processed_count = process_account_id(account_id, params)
